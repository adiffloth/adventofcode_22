...
def get_init_stacks(lines, max_init_height, num_stacks):
    ...
    print(type(stacks), type(stacks[0]))
    return stacks


def get_init_stacks_2(lines, max_init_height, num_stacks):
    stacks = []
    for _ in range(num_stacks):
        stacks.append(str())
...
def part_1_2(lines, max_init_height, num_stacks):
    stacks = get_init_stacks_2(lines, max_init_height, num_stacks)
    for line in lines[max_init_height + 2:]:
        boxes_to_move, source_stack, dest_stack = parse_instructions(line)
        for i in range(boxes_to_move):
            stacks[dest_stack] = stacks[dest_stack] + stacks[source_stack][-1]
            stacks[source_stack] = stacks[source_stack][:-1]
    return get_top_crates(stacks)
