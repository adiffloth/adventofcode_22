...
def solve(puzzle):
    ...
    distances = {(v, li): 1 if li in graph[v] else 1000 for li in graph for v in graph}
    ...
    part2 = max(v1 + v2 for bitm1, v1 in visited2.items() for bitm2, v2 in visited2.items() if not bitm1 & bitm2)
    return part1, part2


time_start = time.perf_counter()
print(solve(read_puzzle('day16/1.in')))
